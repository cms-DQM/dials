...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        df['find_workspace'] = df['path'].apply(lambda x: True
                                                if workspace in x else False)
        df_workspace = df[df['find_workspace']]

        df_melt = df_workspace[[
        ...
        for index, row in df_melt.iterrows():
            ...
            histo = RunHistogram(
                run=
                run,  # would be better with Run.objects.get_or_create(run_number=run_number)
