...
composite_pks = next(filter(lambda x: "primary_key" in x.name, TH1._meta.constraints), None)
...
class TH1ViewSet(GenericViewSetRouter, mixins.ListModelMixin, viewsets.GenericViewSet):
    queryset = TH1.objects.all().order_by(*composite_pks.fields)
    ...
    def get_object(self, request, dataset_id=None, run_number=None, ls_number=None, me_id=None):
        # Since the TH1 table in the database has a composite primary key
