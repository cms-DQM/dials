...
class APIView(View):
    ...
    def initial(self, request, *args, **kwargs):
        ...
        now = time.time()
        logger.debug(f"get_format_suffix {now - start}")
        start = now
        # Perform content negotiation and store the accepted info on the request
        neg = self.perform_content_negotiation(request)
        now = time.time()
        logger.debug(f"negotiation {now - start}")
        start = now
        ...
        now = time.time()
        logger.debug(f"determine version {now - start}")
        start = now
        ...
        now = time.time()
        logger.debug(f"authentication {now - start}")
        start = now
        self.check_permissions(request)
        now = time.time()
        logger.debug(f"check permissions {now - start}")
        start = now
        self.check_throttles(request)
        now = time.time()
        logger.debug(f"check throttles {now - start}")
        start = now
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        now = time.time()
        logger.debug(f"Initialize request {now - start}")
        start = now
        self.request = request
        self.headers = self.default_response_headers  # deprecate?
        now = time.time()
        logger.debug(f"response headers {now - start}")
        start = now
        try:

            self.initial(request, *args, **kwargs)
            now = time.time()
            logger.debug(f"initial {now - start}")
            start = now
            ...
            now = time.time()
            logger.debug(f"get response {now - start}")
            start = now
